    ...
    <div>
      ...
      <div className="table-responsive">
        {templateList && templateList.length > 0 ? (
            ...
            <tbody>
              {templateList.map((template, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  {/* <td>{template.createdDate ? <TextFormat type="date" value={template.createdDate} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{template.createdBy}</td>
                  <td>
                    {template.modifiedDate ? <TextFormat type="date" value={template.modifiedDate} format={APP_DATE_FORMAT} /> : null}
                  </td>
                  ...
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`/template/${template.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/template/${template.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
          ...
          !loading && <div className="alert alert-warning">No Templates found</div>
        )}
      </div>
      {totalItems ? (
